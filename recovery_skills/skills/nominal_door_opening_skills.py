...
class ReachAndGraspHandleSkill(RobotSkill):
    ...
    def precondition_satisfied(self, state, context=None):
        if self.preconds:
            ...
            # check distance between eef and handle
            eef_pos = state['robot_eef:pose/position']
            handle_pos = state['handle:pose/position']
            return handle_pos[1] - eef_pos[1] > 0.1
    ...
    def apply(self, env, obs, context=None, render=False, interpolate=False):
        ...
        handle_theta = obs_dict['handle:pose/theta']

        # this gives the rotation about x axis
        door_theta = obs_dict['door:pose/theta'] + np.pi/2
...
class PullHandleSkill(RobotSkill):
    ...
    def apply(self, env, obs, context=None, render=False, interpolate=False):
        ...
        # pull_distance = 0.20
        open_door_by_angle = np.pi/6
        # pull_distance_y = 0.3
        pull_distance_y = 0.15
        pull_distance_x = pull_distance_y * np.sin(open_door_by_angle)

        current_ee_T = RigidTransform(translation=ee_pos,
                              rotation=T.quat2mat(ee_quat))

        target_pos = np.array([ee_pos[0] - pull_distance_x,
                               ee_pos[1] - pull_distance_y,
                               ee_pos[2]
                               ])
        ee_mat = T.quat2mat(ee_quat)
        # to ensure the latch doesn't close when pulling
        mat_y = mat_about_y(-np.pi/8)
        mat_z = mat_about_z(rotate_by_angle)
        rotate_mat = np.matmul(mat_y, mat_z)
        ...
        for interp in interps_T[1:]:
            action = np.concatenate([interp.translation, interp.axis_angle, [1]])
            actions.append(action)
